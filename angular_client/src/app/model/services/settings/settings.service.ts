...
import {DataTableVO} from '../../vo/DataTableVO';
...
export class SettingsService {
  ...
  */
  ...
  public sx_data_w:Array<ControlValueVO>=[];
  public sx_data_h:Array<ControlValueVO>=[];
  public sy_data_w:Array<ControlValueVO>=[];
  public sy_data_h:Array<ControlValueVO>=[];
  ...
  public changeM1=(vo:ControlValueVO):void=>{
    let route:RouteVO = this.model.route.clone();
        ...
        if(route.view=='compare'){
          if(vo.id=='w'){
            route.SX = this.sx_data_w[0].id;
            route.SY = this.sy_data_w[0].id;
          }else{
            route.SX = this.sx_data_h[0].id;
            route.SY = this.sy_data_h[0].id;
          }
  ...
  public changeSX_SY=(sx:string,sy:string):void=>{
   // this.logger.log('SETTINGS.changeSX_SY(): sx:'+sx+' sy:'+sy);
    let route:RouteVO = this.model.route.clone();
    route.SX = sx;
    route.SY = sy;
    this.model.setRouteValues(route);
  ...
  private loadMenuDataT2(id:string):void{
   // this.logger.log('loadMenuDataT2()');
    let L:number;
    switch (id) {
      case '3':
  ...
  private onloadMenuDataM4Done=(id:string, data:any):void=>{
    if(data.info==='ok'){
      ...
      const m4_data_ids:Array<string> = [];
        ...
        // *******************************************
        // aizvāktas (aizkomentētas) top vērtības saskaņā ar šo biļeti :
        // https://trello.com/c/QX9EfDdF/11-scatter-top-v%C4%93rt%C4%ABbas

        // *******************************************
         //dirsā. šis salauž kartes vizualizācijas un leģendu.
        // *******************************************
...
name_lv_short_loc: "Ventspils n."
  ...
  private onloadMenuDataT2Preloaded(id:string):void{
   ...
   // this.logger.log('SETTINGS - onloadMenuDataT2Preloaded()');
    this.t2_loading = false;
  }
  ...
  public loadTerritoryName(code:string,level:string,year:number):Observable<any>{
    const url:string = this.model.config.serviceURL+'?db=menu-territory-name&level='+level+'&code='+code+'&year='+year;
   // this.logger.log('menu-data-service - loadTerritoryName - url:'+url);
    return this.http.get(url);
  }

  public loadScatterData():void{
    this.logger.log('SETTINGS - loadScatterData');
    this.scatter_loading = true;
    const URL:string = this.model.config.serviceURL+'?db=menu-scatter-values';
    this.http.get(URL).subscribe((data:any) => this.onLoadScatterDataDone({...data}));
  }
  private onLoadScatterDataDone(ob:any):void{
    this.logger.log('*************************************');
    this.logger.log('SETTINGS - onLoadScatterDataDone');
    this.logger.log('*************************************');
    //this.logger.dir(ob);
    this.logger.log('*************************************');
    if(ob.info=='ok' && ob.data.length>0){
      ...
      ob.data.forEach((item:any)=>{
        ...
        if(vo.axis=='x'){
          ...
          if(vo.display=='w'){
            this.sx_data_w.push(co);
          }else{
            this.sx_data_h.push(co);
          }

        }else if(vo.axis=='y'){
          this.sy_data.push(co);
          if(vo.display=='w'){
            this.sy_data_w.push(co);
          }else{
            this.sy_data_h.push(co);
          }
        }
      ...
      this.logger.log('w: *************************************');
      this.logger.dir(this.sx_data_w);
      this.logger.dir(this.sy_data_w);
      this.logger.log('h: *************************************');
      this.logger.dir(this.sx_data_h);
      this.logger.dir(this.sy_data_h);
      this.logger.log('*************************************');
      this.onServiceChange.emit('update');
