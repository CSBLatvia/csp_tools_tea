...
@Injectable()
export class VizCirclesSectorsRegionService  implements IVizService{

  public parent:VizCircleSectorVO = null;
  ...
  public parseData(obj:any):void{
    console.log('***********************');
    console.log('VIZ-CIRCLES-SECTORS-REGION - parseData');
    ...
    let color: string;
      ...
      data.forEach((item:any)=>{
        if(item.type==='flow' && item.property_id) {
          if (item.value !== null && parseInt(item.value) > 0 && this.ids.indexOf(item.code) === -1) {
        ...
        }else if(item.type==='parent' && item.property_id){
          const property_id:string = item.property_id; // display_property_id;
          color = this.model.config.colorCategories.getColorByProperty(property_id);
          this.parent = new VizCircleSectorVO(item.code, parseFloat(item.value + ''), parseFloat(item.value_total + ''), parseFloat(item.choro + ''), property_id, true);
          vo = new VizCircleSectorVO('parent-'+item.code, parseFloat(item.value + ''), parseFloat(item.value_total + ''), parseFloat(item.choro + ''), property_id, true);
          vo.property_color = color;
          vo.picking_color = picking.color();

          max = Math.max(max,vo.value_total);
          this.data.push(vo);
          this.ids.push('parent-'+item.code);
          this.colors.push(vo.picking_color);
        }
      });
    //////////////////
