...
class MenuTerritoryName{
	...
	function getList() {
	    if (!isset($_GET['level'])  || !isset($_GET['year'])  || !isset($_GET['code']) ) {
        ...
        $ps = $this->conn->prepare('SELECT * FROM tea_dev.get_menu_territories(:level, :year, :code)');
        $results = $ps->execute([
                        ':level' => $level,
                        ':year' => $year,
                        ':code' => $code,
