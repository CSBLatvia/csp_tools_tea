...
class ScatterPlot {
	...
	function getList() {
        $requiredParams = ['lang', 'year', 'm1', 'm2', 'm3', 'm4', 't1', 't2', 'sx', 'sy'];
        ...
        $lang = validateString($_GET['lang']);
        $year = validateNumber($_GET['year']);

		$m1 = validateString($_GET['m1']);
		$m2 = validateString($_GET['m2']);
		$m3 = validateStringNotNone($_GET['m3']);
        $m4 = validateStringNotNone($_GET['m4']);

		$t1 = validateNumber($_GET['t1']);  //3,4,7
		$t2 = validateString($_GET['t2']);  //code

		$sx = validateString($_GET['sx']);  //scatter_x
        $sy = validateString($_GET['sy']);  //scatter_y
        ...
        $ps = $this->conn->prepare('SELECT r_t from tea_dev.a_generate_scatter_plot(:lang, :year, :m1, :m2, :m3, :m4, :t1, :t2, :sx, :sy)');
        $results = $ps->execute([
                        ':lang' => $lang,
                        ':year' => $year,
                        ':m1' => $m1,
        ...
        $sql = "SELECT r_t from tea_dev.a_generate_scatter_plot('".$lang."', '".$year."', '".$m1."', '".$m2."', '".$m3."', '".$m4."', '".$t1."', '".$t2."', '".$sx."', '".$sy."')";
