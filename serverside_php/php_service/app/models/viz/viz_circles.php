...
class VizCircles {
	...
	function getList() {
		if (!isset($_GET['m1']) || !isset($_GET['m2']) || !isset($_GET['year']) || !isset($_GET['t1']) ) {
		    errorResponse(StatusCodes::ERROR_WRONG_GET_PARAMS);
		    return;
		}
        //select * from tea_dev.a_generate_map_viz('lv', 2017, 'w', 'av', 'none', 'none', 3, 'all');
        //select * from tea_dev.a_generate_map_viz(lang, year, M1, M2, M3, M4, T1, T2);
        ...
        $ps = $this->conn->prepare('SELECT r_t from tea_dev.a_generate_map_viz(:lang, :year, :m1, :m2, :m3, :m4, :t1, :t2)');
        $results = $ps->execute([
                        ':lang' => $lang,
                        ':year' => $year,
                        ':m1' => $m1,
